...
heuristique(U,H) :-
  ...
  heuristique2(U, H).  % ensuite utilisez plutot l heuristique 2  
   ...
   coordonnees(Piece,Etat,Lig,Col):- nth1(Lig,Etat,Ligne), nth1(Col,Ligne,Piece).
   ...
   malplace(P,U,F):-coordonnees(P,U,Lig,Col),coordonnees(Aux,F,Lig,Col), P\=Aux.
   ...
   % Definir enfin l heuristique qui détermine toutes les pièces mal placées (voir prédicat findall) 
    ...
    heuristique1(U, H) :- final_state(F),findall(Y,(malplace(Y,U,F),Y\=vide),List), length(List,H).
   ...
   dm(P,U,K):-final_state(F),coordonnees(P,U,L,C),coordonnees(P,F,Lf,Cf), Sl is abs(L-Lf), Sc is abs(C-Cf) , K is Sl+Sc .
   
   % Somme des distances de Manhattan à parcourir par chaque piece
   % entre sa position courante et sa positon dans l etat final 

   
   heuristique2(U, H) :- findall(X,(dm(P,U,X),(P\=vide)),List), foldl(plus,List,0,H).
